...
class PublishDeckJob implements ShouldQueue
{
    ...
    public function handle(TGCService $tgc): void
    {
        ...
        // ── Step 4: Composite and upload tuckbox image ───────────────────
try {
    $this->setStatus($jobId, 'running', 'Compositing tuckbox image...');

    // Load character blobs from order_item_cards
    $characterBlobs = [];
    foreach ($order->orderItems as $item) {
        foreach ($item->cards as $card) {
            if ($card->card_type === 'deck' && $card->image_blob) {
                $characterBlobs[] = $card->image_blob;
                break; // one per item — first card only
            }
        }
        if (count($characterBlobs) > 0) break;
    }

    // Get all unique character blobs in order
    $characterBlobs = [];
    foreach ($order->orderItems as $item) {
        foreach ($item->cards as $card) {
            if ($card->card_type === 'deck' && $card->image_blob) {
                $characterBlobs[$card->rank] = $card->image_blob;
            }
        }
    }
