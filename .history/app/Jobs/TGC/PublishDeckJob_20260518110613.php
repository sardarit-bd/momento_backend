...
class PublishDeckJob implements ShouldQueue
{
    ...
    public function handle(TGCService $tgc): void
    {
        ...
        $skuId = data_get($someResponse, 'result.sku_id');

        // If it's null, does your code throw? Or silently continue?
        // Add this guard right after it's assigned:
        if (!$skuId) {
            Log::error('SKU ID is null after deck creation', ['response' => $someResponse]);
            $this->setStatus($jobId, 'failed', 'SKU ID not returned from TGC');
            $this->cleanup($jobId);
            return;
        }
