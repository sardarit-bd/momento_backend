...
class TuckBoxCompositeService
{
    ...
    private const Z3 = ['top' => 0.28, 'left' => 0.51, 'w' => 0.08, 'h' => 0.45];
...
imagealphablending($scaled, false);  // ← false first to fill transparent
imagesavealpha($scaled, true);
$trans = imagecolorallocatealpha($scaled, 0, 0, 0, 127);
imagefill($scaled, 0, 0, $trans);
imagealphablending($scaled, true);  // ← then true to draw on top
imagecopyresampled($scaled, $img, 0, 0, 0, 0, $scaledW, $scaledH, $srcW, $srcH);

            // Apply clipPath if present
            if (!empty($slot['clip'])) {
                $scaled = $this->applyInsetClip($scaled, $scaledW, $scaledH, $slot['clip']);
            }

            imagecopy($canvas, $scaled, $scaledX, $scaledY, 0, 0, $scaledW, $scaledH);
            imagedestroy($scaled);
        }
    }
