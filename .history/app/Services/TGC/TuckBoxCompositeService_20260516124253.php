    ...
    private function drawZone2($canvas, array $imgs, int $W, int $H, int $total): void
{
    ...
    foreach ($layout as $slot) {
        ...
        // Base center: bottom-center of zone
        $baseCx = $zX + (int)($zW / 2);
        $baseCy = $zY + $zH;
        ...
        // NOW position the (possibly clipped) image
        // Center horizontally, anchor at bottom vertically
        $finalW = imagesx($scaled);
        $finalH = imagesy($scaled);

        // x offset: percentage of ZONE width, applied AFTER centering
        $xOffsetPx = (int)(($slot['x'] / 100) * $zW);
        // y offset: percentage of ZONE height, from bottom
        $yOffsetPx = (int)(($slot['y'] / 100) * $zH);

        $drawX = $baseCx - (int)($finalW / 2) + $xOffsetPx;
        $drawY = $baseCy - $finalH + $yOffsetPx;

        imagecopy($canvas, $scaled, $drawX, $drawY, 0, 0, $finalW, $finalH);
        imagedestroy($scaled);
    }
}
