...
class TuckBoxCompositeService
{
    ...
    private const Z3 = ['top' => 0.10, 'left' => 0.00, 'w' => 0.08, 'h' => 0.25];
    ...
    private function drawZone3($canvas, array $imgs, int $W, int $H, int $total): void
    {
        ...
        foreach ($layout as $key => $slot) {
            ...
            // ── STEP 2: Rotate 90° for side panel ──
            $bgColor = imagecolorallocatealpha($temp, 0, 0, 0, 127);
            $rotated = imagerotate($temp, 90, $bgColor);
            imagesavealpha($rotated, true);

            // Use ACTUAL dimensions — imagerotate may add padding
            $rotW = imagesx($rotated);
            $rotH = imagesy($rotated);

            // ── STEP 3: Position on the final canvas ──
            // ┌──────────────────────────────────────────────────┐
            // │  $destX = horizontal position on canvas          │
            // │  $destY = vertical position on canvas            │
            // │                                                  │
            // │  To move LEFT on the canvas → decrease $destX    │
            // │  To move RIGHT on the canvas → increase $destX   │
            // │  To move UP on the canvas → decrease $destY      │
            // │  To move DOWN on the canvas → increase $destY    │
            // └──────────────────────────────────────────────────┘

            // After rotation, X and Y are swapped
// To move LEFT/RIGHT on canvas → change $destY (not $destX)
// To move UP/DOWN on canvas → change $destX (not $destY)

$xShift = $isLeader ? -0.80 : -0.90; // ← controls UP/DOWN after rotation
$destX  = (int)($zX + $zW / 2 + $xShift * $itemW);

$yShift = $isLeader ? 0.0 : -0.40;   // ← controls LEFT/RIGHT after rotation
