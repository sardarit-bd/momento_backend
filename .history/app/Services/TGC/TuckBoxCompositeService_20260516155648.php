...
class TuckBoxCompositeService
{
    ...
    // Zone 2 — Front face
    private const Z2 = ['top' => 0.43, 'left' => 0.10, 'w' => 0.36, 'h' => 0.92];

    // Zone 3 — Right side strip
    private const Z3 = ['top' => 0.25, 'left' => 0.00, 'w' => 0.88, 'h' => 0.35];
    ...
    private function drawZone3($canvas, array $imgs, int $W, int $H, int $total): void
    {
        ...
        foreach ($layout as $key => $slot) {
            ...
            // 1. Element dimensions (CSS: width 85/70%, aspect-ratio 1/1)
            $itemW = (int)($zW * ($isLeader ? 0.85 : 0.70));
            $itemH = $itemW;

            // CSS flex: margin-top -12% (In CSS, vertical margin % resolves against the CONTAINER width)
            $marginTop = $key === 0 ? 0 : (int)($zW * -0.12);
            $currentY += $marginTop;

            // 2. Create temp canvas for the element
            $temp = imagecreatetruecolor($itemW, $itemH);
            imagealphablending($temp, false);
            imagesavealpha($temp, true);
            $trans = imagecolorallocatealpha($temp, 0, 0, 0, 127);
            ...
            // 3. Draw image inside element (CSS: width 100%, height 250%, top 0, left -10%)
            $drawW    = $itemW;
            $drawH    = (int)($itemW * 2.50);
            $drawDstX = (int)(-$itemW * 0.10); // left: -10%
            $drawDstY = 0;                     // top: 0%

            imagecopyresampled($temp, $img, $drawDstX, $drawDstY, 0, 0, $drawW, $drawH, $srcW, $srcH);

            // 4. Apply border-radius 40% for non-leaders
            if (!$isLeader) {
                $temp = $this->applyRoundedMask($temp, $itemW, $itemH, 0.40);
            }

            // 5. Rotate -90deg (CSS: rotate(-90deg) is counter-clockwise, which is +90 in PHP GD)
            $rotated = imagerotate($temp, 90, imagecolorallocatealpha($temp, 0, 0, 0, 127));
            imagesavealpha($rotated, true);

            // 6. Calculate exact position matching CSS transforms
            // CSS flex: items-center means the element is horizontally centered in the zone
            $elemX = $zX + (int)(($zW - $itemW) / 2);
            $elemY = $currentY;

            // CSS transform: translateX(-80/-90%) translateY(0/-40%)
            // In CSS, transform percentages are based on the ELEMENT's own size
            $tx = ($isLeader ? -0.80 : -0.90) * $itemW;
            $ty = ($isLeader ? 0.0 : -0.40) * $itemH;

            // Calculate the final center point (CSS transforms apply relative to the center)
            $finalCenterX = $elemX + $itemW / 2 + $tx;
            $finalCenterY = $elemY + $itemH / 2 + $ty;

            // Destination coordinates for the top-left corner
            $destX = (int)($finalCenterX - $itemW / 2);
            $destY = (int)($finalCenterY - $itemH / 2);

            // Draw once, correctly
            imagecopy($canvas, $rotated, $destX, $destY, 0, 0, $itemW, $itemH);
    ...
    private function getZone2Layout(int $total): array
    {
        ...
        return [
            ...
            ['i' => 0, 'x' => 0,   'y' =>  32, 'scale' => 1,    'size' => 1.09, 'z' => 3, 'clip' => '0% 25% 10% 25%'],
        ];
    }
