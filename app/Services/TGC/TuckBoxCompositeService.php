...
class TuckBoxCompositeService
{
    ...
    private function applyRoundedMask($img, int $w, int $h, float $radiusRatio)
    {
        ...
        for ($x = 0; $x < $w; $x++) {
            for ($y = 0; $y < $h; $y++) {
                $inCorner = false;
                if ($x < $rx && $y < $ry && (($x - $rx) ** 2 / $rx ** 2 + ($y - $ry) ** 2 / $ry ** 2) > 1) {
                    $inCorner = true;
                }
                if ($x >= $w - $rx && $y < $ry && (($x - ($w - $rx)) ** 2 / $rx ** 2 + ($y - $ry) ** 2 / $ry ** 2) > 1) {
                    $inCorner = true;
                }
                if ($x < $rx && $y >= $h - $ry && (($x - $rx) ** 2 / $rx ** 2 + ($y - ($h - $ry)) ** 2 / $ry ** 2) > 1) {
                    $inCorner = true;
                }
                if ($x >= $w - $rx && $y >= $h - $ry && (($x - ($w - $rx)) ** 2 / $rx ** 2 + ($y - ($h - $ry)) ** 2 / $ry ** 2) > 1) {
                    $inCorner = true;
                }
